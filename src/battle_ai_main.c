...
// the following checks apply to any target (including user)
        ...
        case EFFECT_MISTY_EXPLOSION:
            ...
            else if (CountUsablePartyMons(battlerAtk) == 0)
            {
                if (CountUsablePartyMons(battlerDef) != 0)
                    ADJUST_AND_RETURN_SCORE(NO_DAMAGE_OR_FAILS);
                // Bazzo note: this might mess up guaranteed boom on 1hp
                    /*
                else
                    ADJUST_SCORE(-1);
                */
    ...
    // stat lowering effects
        ...
        case EFFECT_NOBLE_ROAR:
            if (!CanLowerStat(battlerAtk, battlerDef, aiData, STAT_SPATK))
                ...
                ADJUST_AND_RETURN_SCORE(NO_DAMAGE_OR_FAILS);
            break;
        //Bazzo note: removed as captivate planned to work on both genders
            /*
        case EFFECT_CAPTIVATE:
            ...
            */
    // other
    //Bazzo note: copying this out for new haze ai
    /*
        case EFFECT_HAZE:
    ...
    */
        case EFFECT_HAZE:
            if ((!AnyStatIsRaised(battlerDef))
             && (!AnyStatIsLowered(battlerAtk)))
                ADJUST_AND_RETURN_SCORE(NO_DAMAGE_OR_FAILS);
            break;
        case EFFECT_PRESENT:
        ...
        //case EFFECT_FOCUS_PUNCH:
            ...
            if (aiData->abilities[battlerDef] == ABILITY_WONDER_GUARD && effectiveness < UQ_4_12(2.0))
                ADJUST_AND_RETURN_SCORE(NO_DAMAGE_OR_FAILS);
            // Bazzo note: Removing this as threat of these moves is enough, shouldn't be unnatural ways to discourage them
            /*
        ...
        case EFFECT_SHED_TAIL:
            if (CountUsablePartyMons(battlerAtk) == 0)
                ...
                ADJUST_AND_RETURN_SCORE(NO_DAMAGE_OR_FAILS);
            // Bazzo note: didn't want shed tail to be prevented in same way as sub
            /*
            if (gBattleMons[battlerAtk].volatiles.substitute || aiData->abilities[battlerDef] == ABILITY_INFILTRATOR)
                ADJUST_SCORE(-8);
            else if (HasMoveWithFlag(battlerDef, MoveIgnoresSubstitute))
        ...
        case EFFECT_LEECH_SEED:
            if (gBattleMons[battlerDef].volatiles.leechSeed
             ...
             || DoesPartnerHaveSameMoveEffect(BATTLE_PARTNER(battlerAtk), battlerDef, move, aiData->partnerMove))
                ...
                ADJUST_AND_RETURN_SCORE(NO_DAMAGE_OR_FAILS);
            break;
            // Bazzo note: fuck dealing with this disable ai lol
        case EFFECT_DISABLE:
        ...
        case EFFECT_CURSE:
            if (IS_BATTLER_OF_TYPE(battlerAtk, TYPE_GHOST))
            {
                if (gBattleMons[battlerDef].volatiles.cursed
                  || DoesPartnerHaveSameMoveEffect(BATTLE_PARTNER(battlerAtk), battlerDef, move, aiData->partnerMove))
                    ADJUST_AND_RETURN_SCORE(NO_DAMAGE_OR_FAILS);
                    // Bazzo note: removing this next one - seppuku curse is back on the menu 8) 
                    /*
                else if (aiData->hpPercents[battlerAtk] <= 50)
                    ADJUST_SCORE(-6); */
            }
        ...
        case EFFECT_SPIKES:
            if (gSideTimers[GetBattlerSide(battlerDef)].spikesAmount >= 1)
              ...
              && gSideTimers[GetBattlerSide(battlerDef)].spikesAmount == 2)
                ADJUST_SCORE(-10); // only one mon needs to set up the last layer of Spikes - Bazzo note: this won't apply with above score changed to one layer
            break;
        ...
        case EFFECT_SWALLOW:
            ...
            else
            {
                if (AI_BattlerAtMaxHp(battlerAtk))
                    ...
                    ADJUST_AND_RETURN_SCORE(NO_DAMAGE_OR_FAILS); // do it if nothing better
            }
            break;
        case EFFECT_TORMENT:
            if (GetActiveGimmick(battlerDef) == GIMMICK_DYNAMAX)
        ...
        case EFFECT_WATER_SPORT:
            if (gFieldStatuses & STATUS_FIELD_WATERSPORT
              ...
              || DoesPartnerHaveSameMoveEffect(BATTLE_PARTNER(battlerAtk), battlerDef, move, aiData->partnerMove))
                ADJUST_AND_RETURN_SCORE(NO_DAMAGE_OR_FAILS);
            break;
            // Bazzo note: Changed this to no increase
        case EFFECT_ABSORB:
        ...
        case EFFECT_ROOST:
            if (AI_BattlerAtMaxHp(battlerAtk))
            {
                DebugPrintf("print FAIL 2");
                ADJUST_AND_RETURN_SCORE(NO_DAMAGE_OR_FAILS);
            }
            //else if (aiData->hpPercents[battlerAtk] >= 90)
            //    ADJUST_AND_RETURN_SCORE(NO_DAMAGE_OR_FAILS); //No point in healing, but should at least do it if nothing better
            break;
        ...
        case EFFECT_PURIFY:
            ...
            break;
            // Bazzo note: removing this. ai you gotta man up buddy SEND IT!!!
            /*
        ...
        case EFFECT_LASER_FOCUS:
            ...
            || aiData->abilities[battlerDef] == ABILITY_LEAF_GUARD || aiData->abilities[battlerDef] == ABILITY_MAGMA_ARMOR)
                ADJUST_AND_RETURN_SCORE(NO_DAMAGE_OR_FAILS);
            break;
        case EFFECT_SKETCH:
            if (aiData->lastUsedMove[battlerDef] == MOVE_NONE)
                ADJUST_AND_RETURN_SCORE(NO_DAMAGE_OR_FAILS);
            break;
            // TODO commenting this out assuming that destiny bond will be changed back to the old version
            /*
        ...
        // Bazzo note: removed, not intuitive
            /*
        ...
        case EFFECT_TAUNT:
            if (gDisableStructs[battlerDef].tauntTimer > 0
              || DoesPartnerHaveSameMoveEffect(BATTLE_PARTNER(battlerAtk), battlerDef, move, aiData->partnerMove))
              //|| !HasMoveWithCategory(battlerDef, DAMAGE_CATEGORY_STATUS))
                ADJUST_AND_RETURN_SCORE(NO_DAMAGE_OR_FAILS);
            break;
        case EFFECT_BESTOW:
        ...
        case EFFECT_POWER_SPLIT:
            ...
            break;
            // Bazzo note: taking this out as it would cause Blissey to sometimes not use it where it should
            /*
            ...
            if (!IsTargetingPartner(battlerAtk, battlerDef))
            {
                ...
                if (targetPositiveStages == 0 //No good stat changes to make bad
                  || PartnerMoveIsSameAsAttacker(BATTLE_PARTNER(battlerAtk), battlerDef, move, aiData->partnerMove))
                    ...
                    ADJUST_AND_RETURN_SCORE(NO_DAMAGE_OR_FAILS); //More stages would be made positive than negative
            }
            break;
        ...
        case EFFECT_AFTER_YOU:
            if (!IsTargetingPartner(battlerAtk, battlerDef)
              ...
              || PartnerMoveIsSameAsAttacker(BATTLE_PARTNER(battlerAtk), battlerDef, move, aiData->partnerMove))
                ADJUST_SCORE(-10);
            break;
            // Bazzo Note: replaced ai with check for fail last time (shoutout grint the goat again) - temporarily removing
        ...
        if ((gLastMoves[battlerAtk] == move) && (gBattleStruct->battlerState[battlerAtk].suckerPunchTimer > 0) && RandomPercentage(RNG_AI_CUSTOM_AI_FIFTY_PERCENT, CUSTOM_AI_FIFTY_PERCENT))
                    ADJUST_AND_RETURN_SCORE(NO_DAMAGE_OR_FAILS);
                break;
        ...
        case EFFECT_SYNCHRONOISE:
            ...
            if (aiData->holdEffects[battlerDef] == HOLD_EFFECT_RING_TARGET
              || DoBattlersShareType(battlerAtk, battlerDef))
                ...
                ADJUST_AND_RETURN_SCORE(NO_DAMAGE_OR_FAILS);
            break;
            // Bazzo note: no reason to discourage flail so removing this
            /*
        case EFFECT_FLAIL:
        ...
        case EFFECT_SPICY_EXTRACT:
            if (battlerAtk != BATTLE_PARTNER(battlerDef)
             && (HasMoveWithCategory(battlerDef, DAMAGE_CATEGORY_PHYSICAL)
              ...
              || aiData->abilities[battlerDef] == ABILITY_FLOWER_BLOCK
...
{ 
    ...
    if (CanIndexMoveFaintTarget(battlerAtk, battlerDef, movesetIndex, AI_ATTACKING)
        && effect != EFFECT_EXPLOSION && effect != EFFECT_MISTY_EXPLOSION && effect != EFFECT_FINAL_GAMBIT && effect != EFFECT_SHELL_TRAP && effect != EFFECT_ENDEAVOR)
    { 
    ...
    { 
        if (RandomPercentage(RNG_AI_PRIORITIZE_LAST_CHANCE, PRIORITIZE_LAST_CHANCE_CHANCE))
            ADJUST_SCORE(LAST_CHANCE); // Bazzo note: changed to be same, used to be slow kill + 2 - Don't outscore Fast Kill (which gets a bonus point in AI_CompareDamagingMoves), but do outscore Slow Kill getting the same
        else
...
static s32 AI_DoubleBattle(u32 battlerAtk, u32 battlerDef, u32 move, s32 score)
{
    ...
    case EFFECT_HELPING_HAND:
        ...
        else
        {
            ...
            if (hasTwoOpponents)
            {
                ...
                else ADJUST_SCORE(BEST_DAMAGE_MOVE);
            }
            else if (IsBattlerAlive(LEFT_FOE(battlerAtk)))
            {
                ...
                if (CanTargetFaintAi(LEFT_FOE(battlerAtk), battlerAtk)
                 && AI_IsSlower(battlerAtk, LEFT_FOE(battlerAtk), move, predictedMove, DONT_CONSIDER_PRIORITY))
                    ADJUST_SCORE(BEST_DAMAGE_MOVE + SLOW_KILL + 2);
            ...
            else if (IsBattlerAlive(RIGHT_FOE(battlerAtk)))
            {
                ...
                else if (ownHitsToKOFoe2 > partnerHitsToKOFoe2 && partnerHitsToKOFoe2 > 1)
                    ADJUST_SCORE(BEST_DAMAGE_MOVE + 1);
    ...
    case EFFECT_MAGNET_RISE:
        ...
        {
            RETURN_SCORE_PLUS(GOOD_EFFECT);   // partner has earthquake or magnitude -> good idea to use magnet rise
        }
        break;
    case EFFECT_DRAGON_CHEER:
        if (gBattleMons[battlerAtkPartner].volatiles.dragonCheer
         ...
         || !HasDamagingMove(battlerAtkPartner))
            ...
            ADJUST_SCORE(BEST_DAMAGE_MOVE + 1);
        else if ( !IS_BATTLER_OF_TYPE(battlerAtkPartner, TYPE_DRAGON)
              || !gBattleMons[battlerAtk].volatiles.dragonCheer
              || !gBattleMons[battlerAtkPartner].volatiles.focusEnergy)
            ADJUST_SCORE(BEST_DAMAGE_MOVE);
        break;
    ...
    */
    if (IsTargetingPartner(battlerAtk, battlerDef))
    {
        if (!hasPartner
         || !HasMoveWithCategory(battlerAtkPartner, DAMAGE_CATEGORY_PHYSICAL)
         || CanTargetFaintAi(BATTLE_OPPOSITE(battlerAtk), battlerAtkPartner)
         || CanTargetFaintAi(BATTLE_OPPOSITE(battlerAtkPartner), battlerAtkPartner)
         || gBattleMons[battlerAtkPartner].statStages[STAT_ATK] >= DEFAULT_STAT_STAGE + 2
    ...
    case EFFECT_TAILWIND:
        ...
        if (speed + partnerSpeed < foe1Speed + foe2Speed)
    ...
    case EFFECT_PROTECT:
        ...
        enum ProtectMethod protectMethod = GetMoveProtectMethod(move);
        //DebugPrintf("going through doubles ai");
        switch (protectMethod)
        { 
            case PROTECT_MAT_BLOCK:
                break;
...
/*
    ...
    // Bazzo note: getting rid of the next two as this is silly lol
...
*/
        ...
        if (wouldPartnerFaint)
        {
                ...
                && CanIndexMoveFaintTarget(battlerAtk, BATTLE_OPPOSITE(battlerAtkPartner), gAiThinkingStruct->movesetIndex, AI_ATTACKING))
            {
                ...
                case EFFECT_MISTY_EXPLOSION:
                    ...
                    {
                        RETURN_SCORE_PLUS(10); // Bazzo note: risky ai so doesn't matter
                    }
                    break;
                default:
                    ADJUST_SCORE(+3);
            ...
            // Bazzo note: removing this as it doesn't seem relevant/too tricky for no reason. As long as ai sees a kill with spread move, it should be encouraged
            /*
            ...
            {               
                ...
                case ABILITY_VOLT_ABSORB:
                    if (moveType == TYPE_ELECTRIC)
                    {
                        ...
                        //Bazzo TODO: add something here so that if the ability is volt absorb and ai is full hp, it doesn't get a boost. same with stat boosters and max stats

                        if (atkPartnerAbility == ABILITY_VOLT_ABSORB && AI_BattlerAtMaxHp(battlerAtkPartner))
                                {
                                ADJUST_SCORE(NO_INCREASE);
                                }
                ...
                case ABILITY_LEVITATE:
                    ...
                    {
                        if (atkPartnerAbility == ABILITY_EARTH_EATER && AI_BattlerAtMaxHp(battlerAtkPartner))
                                {
                                ADJUST_SCORE(NO_INCREASE);
                                }
                        else if (moveTarget == MOVE_TARGET_FOES_AND_ALLY)
                ...
                if (moveType == TYPE_WATER)
                    {
                        ...
                        else if (atkPartnerAbility == ABILITY_WATER_ABSORB && AI_BattlerAtMaxHp(battlerAtkPartner))
                        {
                            ADJUST_SCORE(NO_INCREASE);
                        }
                        else if (moveTarget == MOVE_TARGET_FOES_AND_ALLY)
    ...
    //Bazzo note: the below is separate to the above checks in that the above is changing score for targeting player, whereas below is for targeting ai's partner
    ...
    if (IsTargetingPartner(battlerAtk, battlerDef))
    {
        ...
        if (!partnerProtecting && moveTarget != MOVE_TARGET_BOTH && !DoesBattlerIgnoreAbilityChecks(battlerAtk, aiData->abilities[battlerAtk], move))
        {
            ...
            case ABILITY_VOLT_ABSORB:
                if (moveType == TYPE_ELECTRIC)
                {
                    ...
                    //Bazzo TODO: add something here so that if the ability is volt absorb and ai is full hp, it doesn't get a boost DONE. same with stat boosters and max stats

                    if ((moveTarget == MOVE_TARGET_FOES_AND_ALLY)
                        && atkPartnerAbility == ABILITY_VOLT_ABSORB
            ...
            case ABILITY_JUSTIFIED:
                    ...
                    && ShouldTriggerAbility(battlerAtk, battlerAtkPartner, atkPartnerAbility))
                {
                    ...
                    ADJUST_SCORE(NO_INCREASE); // Beat Up is handled later
                }
                else
                {
                    isMoveAffectedByPartnerAbility = FALSE;
            ...
            case ABILITY_RATTLED:
                ...
                break;
                // DECENTEFFECTCONTRARYSWAP
                // Bazzo note: come back here when you're a pro because wtf is skill swap ai doing LOL
            case ABILITY_CONTRARY:
        ...
        if (!partnerProtecting)
        {
            ...
            case EFFECT_SPICY_EXTRACT:
                ...
                {
                    RETURN_SCORE_PLUS(BEST_DAMAGE_MOVE - 1);
                }
                else
                {
                    RETURN_SCORE_PLUS(GOOD_EFFECT);  
            ...
            case EFFECT_DECORATE:
                if (gBattleMons[battlerAtkPartner].statStages[STAT_ATK] > DEFAULT_STAT_STAGE
                || gBattleMons[battlerAtkPartner].statStages[STAT_SPATK] > DEFAULT_STAT_STAGE)
                {
                    RETURN_SCORE_PLUS(BEST_DAMAGE_MOVE - 1)
                }
                else
            ...
            case EFFECT_INSTRUCT:
                {
                    ...
                    if (AI_IsFaster(battlerAtk, battlerAtkPartner, move, predictedMoveSpeedCheck, CONSIDER_PRIORITY))
                        ...
                        instructedMove = gBattleMons[BATTLE_PARTNER(battlerAtk)].moves[gAiBattleData->chosenMoveIndex[BATTLE_PARTNER(battlerAtk)]];
                    ...
                    //  && (GetBattlerMoveTargetType(battlerAtkPartner, instructedMove) & (MOVE_TARGET_BOTH | MOVE_TARGET_FOES_AND_ALLY))) // Use instruct on multi-target moves
                      ...
                      // Bazzo note: commenting this last bit out because it's silly && (GetBattlerMoveTargetType(battlerAtkPartner, instructedMove) & (MOVE_TARGET_BOTH | MOVE_TARGET_FOES_AND_ALLY))) // Use instruct on multi-target moves
                    {
            ...
            case EFFECT_AFTER_YOU:
                 ...
                 && AI_IsSlower(battlerAtkPartner, RIGHT_FOE(battlerAtk), aiData->partnerMove, predictedMoveSpeedCheck, CONSIDER_PRIORITY)) // Opponent mon 2 goes before partner
                {
                    ...
                    ADJUST_SCORE(BEST_DAMAGE_MOVE); // Bazzo note: changed this to +6 temporarily so after you trick room test works
                }
                break;
            ...
            case EFFECT_TELEKINESIS:
                ...
                break;
            case EFFECT_TOPSY_TURVY:
            DebugPrintf("Atk: %d partner: %d partnerMove: %S", battlerAtk, BATTLE_PARTNER(battlerAtk), GetMoveName(aiData->partnerMove));
            DebugPrintf("isSlower %d", AI_IsSlower(battlerAtk, BATTLE_PARTNER(battlerAtk), move, predictedMoveSpeedCheck, DONT_CONSIDER_PRIORITY));
                if (AI_IsSlower(battlerAtk, BATTLE_PARTNER(battlerAtk), move, predictedMoveSpeedCheck, DONT_CONSIDER_PRIORITY))
                { DebugPrintf("print 1");
                    if (MoveHasAdditionalEffectSelf(gBattleMons[BATTLE_PARTNER(battlerAtk)].moves[gAiBattleData->chosenMoveIndex[BATTLE_PARTNER(battlerAtk)]], MOVE_EFFECT_DEF_SPDEF_DOWN))
                    {   DebugPrintf("print additional effect check");
                        if (RandomPercentage(RNG_AI_CUSTOM_AI_FIFTY_PERCENT, CUSTOM_AI_FIFTY_PERCENT))
                            ADJUST_SCORE(DECENT_EFFECT);
                        else 
                            ADJUST_SCORE(BEST_DAMAGE_MOVE);
                ...
                else if (AnyStatIsLowered(BATTLE_PARTNER(battlerAtk)))
                { DebugPrintf("print 2");
                    if (RandomPercentage(RNG_AI_CUSTOM_AI_FIFTY_PERCENT, CUSTOM_AI_FIFTY_PERCENT))
                            ADJUST_SCORE(DECENT_EFFECT);
                        else 
    ...
    else // checking opponent
    {
        ...
        // Bazzo note: changed ai stealing ability outside of having traunt to no increase because it was fucking up the test, so if you want this to be a thing, change that back
        switch (effect)
        {
        case EFFECT_SKILL_SWAP:
            if (aiData->abilities[battlerAtk] == ABILITY_TRUANT)
                ...
                ADJUST_SCORE(NO_INCREASE); // we want to transfer our bad ability or take their awesome ability DECENTEFFECTCONTRARYSWAP THE PROBLEM WAS HERE!!!!!!!!!!!!!
            break;
        default:
            break;
        }
...
// Bazzo note: this also looks like it should be removed if there's nothing important in there, moves with the same scores should just be clicked at random. 
// copying out as its now unused according to terminal
...
//Bazzo note: taking this out because it's silly
/*
...
static s32 AI_CalcMoveEffectScore(u32 battlerAtk, u32 battlerDef, u32 move, struct AiLogicData *aiData)
{
    ...
    u32 AIHitsToKOPlayerMaxRoll = GetBestNoOfHitsToKO(battlerAtk, battlerDef, AI_ATTACKING_MAX_ROLL);
    u32 PlayerHitsToKOAIMaxRoll = GetBestNoOfHitsToKO(battlerDef, battlerAtk, AI_DEFENDING_MAX_ROLL);
...
*/
    ...
    */
    ...
    case EFFECT_MIRROR_MOVE:
        ...
        break;
    case EFFECT_ATTACK_UP_USER_ALLY:
        ADJUST_SCORE(IncreaseStatUpScore(BATTLE_PARTNER(battlerAtk), battlerDef, STAT_CHANGE_ATK, move));
    case EFFECT_ATTACK_UP:
        ADJUST_SCORE(IncreaseStatUpScore(battlerAtk, battlerDef, STAT_CHANGE_ATK, move));
        break;
    case EFFECT_ATTACK_UP_2:
        ADJUST_SCORE(IncreaseStatUpScore(battlerAtk, battlerDef, STAT_CHANGE_ATK_2, move));
        break;
    case EFFECT_DEFENSE_UP:
        ADJUST_SCORE(IncreaseStatUpScore(battlerAtk, battlerDef, STAT_CHANGE_DEF, move));
        break;
    case EFFECT_DEFENSE_UP_3:
        ADJUST_SCORE(IncreaseStatUpScore(battlerAtk, battlerDef, STAT_CHANGE_DEF_3, move));
        break;
    case EFFECT_STUFF_CHEEKS:
    case EFFECT_DEFENSE_UP_2:
        ADJUST_SCORE(IncreaseStatUpScore(battlerAtk, battlerDef, STAT_CHANGE_DEF_2, move));
        break;
    case EFFECT_SPEED_UP:
        ADJUST_SCORE(IncreaseStatUpScore(battlerAtk, battlerDef, STAT_CHANGE_SPEED, move));
        break;
    case EFFECT_AUTOTOMIZE:
    case EFFECT_SPEED_UP_2:
        ADJUST_SCORE(IncreaseStatUpScore(battlerAtk, battlerDef, STAT_CHANGE_SPEED_2, move));
        break;
    case EFFECT_SPECIAL_ATTACK_UP:
        ADJUST_SCORE(IncreaseStatUpScore(battlerAtk, battlerDef, STAT_CHANGE_SPATK, move));
        break;
    case EFFECT_SPECIAL_ATTACK_UP_2:
        ADJUST_SCORE(IncreaseStatUpScore(battlerAtk, battlerDef, STAT_CHANGE_SPATK_2, move));
        break;
    case EFFECT_SPECIAL_ATTACK_UP_3:
        ADJUST_SCORE(IncreaseStatUpScore(battlerAtk, battlerDef, STAT_CHANGE_SPATK_3, move));
        break;
    case EFFECT_SPECIAL_DEFENSE_UP:
        ADJUST_SCORE(IncreaseStatUpScore(battlerAtk, battlerDef, STAT_CHANGE_SPDEF, move));
        break;
    case EFFECT_SPECIAL_DEFENSE_UP_2:
        ADJUST_SCORE(IncreaseStatUpScore(battlerAtk, battlerDef, STAT_CHANGE_SPDEF_2, move));
        break;
    case EFFECT_ACCURACY_UP:
    case EFFECT_ACCURACY_UP_2:
        ADJUST_SCORE(IncreaseStatUpScore(battlerAtk, battlerDef, STAT_CHANGE_ACC, move));
        break;
    case EFFECT_EVASION_UP:
    case EFFECT_EVASION_UP_2:
        ADJUST_SCORE(IncreaseStatUpScore(battlerAtk, battlerDef, STAT_CHANGE_EVASION, move));
        break;
    case EFFECT_ATTACK_DOWN:
        ADJUST_SCORE(IncreaseStatDownScore(battlerAtk, battlerDef, STAT_CHANGE_ATK, move));
        break;
    case EFFECT_ATTACK_DOWN_2:
        ADJUST_SCORE(IncreaseStatDownScore(battlerAtk, battlerDef, STAT_CHANGE_ATK_2, move));
        break;
    case EFFECT_DEFENSE_DOWN:
        ADJUST_SCORE(IncreaseStatDownScore(battlerAtk, battlerDef, STAT_CHANGE_DEF, move));
        break;
    case EFFECT_DEFENSE_DOWN_2:
        ADJUST_SCORE(IncreaseStatDownScore(battlerAtk, battlerDef, STAT_CHANGE_DEF_2, move));
        break;
    case EFFECT_SPEED_DOWN:
        ADJUST_SCORE(IncreaseStatDownScore(battlerAtk, battlerDef, STAT_CHANGE_SPEED, move));
        break;
    case EFFECT_SPEED_DOWN_2:
        ADJUST_SCORE(IncreaseStatDownScore(battlerAtk, battlerDef, STAT_CHANGE_SPEED_2, move));
        break;
    case EFFECT_SPECIAL_ATTACK_DOWN:
        ADJUST_SCORE(IncreaseStatDownScore(battlerAtk, battlerDef, STAT_CHANGE_SPATK, move));
        break;
    case EFFECT_SPECIAL_ATTACK_DOWN_2:
        ADJUST_SCORE(IncreaseStatDownScore(battlerAtk, battlerDef, STAT_CHANGE_SPATK_2, move));
        break;
    case EFFECT_SPECIAL_DEFENSE_DOWN:
        ADJUST_SCORE(IncreaseStatDownScore(battlerAtk, battlerDef, STAT_CHANGE_SPDEF, move));
        break;
    case EFFECT_SPECIAL_DEFENSE_DOWN_2:
        ADJUST_SCORE(IncreaseStatDownScore(battlerAtk, battlerDef, STAT_CHANGE_SPDEF_2, move));
        break;
    case EFFECT_ACCURACY_DOWN:
    case EFFECT_ACCURACY_DOWN_2:
        ADJUST_SCORE(IncreaseStatDownScore(battlerAtk, battlerDef, STAT_CHANGE_ACC, move));
        break;
    case EFFECT_EVASION_DOWN:
    case EFFECT_EVASION_DOWN_2:
        ADJUST_SCORE(IncreaseStatDownScore(battlerAtk, battlerDef, STAT_CHANGE_EVASION, move));
        break;
    case EFFECT_SPICY_EXTRACT:
    ...
    case EFFECT_GEAR_UP:
        ...
        {
            ADJUST_SCORE(IncreaseStatUpScore(BATTLE_PARTNER(battlerAtk), battlerDef, STAT_CHANGE_ATK, move));
            ADJUST_SCORE(IncreaseStatUpScore(BATTLE_PARTNER(battlerAtk), battlerDef, STAT_CHANGE_SPATK, move));
        }
        break;
    case EFFECT_MAGNETIC_FLUX:
        if (aiData->abilities[battlerAtk] == ABILITY_PLUS || aiData->abilities[battlerAtk] == ABILITY_MINUS)
        ...
        if (hasPartner && (aiData->abilities[BATTLE_PARTNER(battlerAtk)] == ABILITY_PLUS || aiData->abilities[BATTLE_PARTNER(battlerAtk)] == ABILITY_MINUS))
        {
            ADJUST_SCORE(IncreaseStatUpScore(BATTLE_PARTNER(battlerAtk), battlerDef, STAT_CHANGE_DEF, move));
            ADJUST_SCORE(IncreaseStatUpScore(BATTLE_PARTNER(battlerAtk), battlerDef, STAT_CHANGE_SPDEF, move));
        }
        break;
    case EFFECT_ATTACK_ACCURACY_UP: // hone claws
        ADJUST_SCORE(IncreaseStatUpScore(battlerAtk, battlerDef, STAT_CHANGE_ATK, move));
    ...
    */
        ...
        || AnyStatIsLowered(battlerAtk))
            { 
                ...
                && (!AnyStatIsLowered(battlerDef)))
                {
                    ADJUST_SCORE(BEST_DAMAGE_MOVE);
                }
            ADJUST_SCORE(NO_INCREASE);
            }
        //else if ((!AnyStatIsRaised(battlerDef))
          //    && (!AnyStatIsLowered(battlerAtk)))
            //    ADJUST_AND_RETURN_SCORE(NO_DAMAGE_OR_FAILS);
            break;
    case EFFECT_ROAR:
    ...
    case EFFECT_LIFE_DEW:
        {
        if  
            (ShouldRecover(battlerAtk, battlerDef, move, 25))
            ADJUST_SCORE(BEST_DAMAGE_MOVE);
        else if (ShouldRecover(BATTLE_PARTNER(battlerAtk), battlerDef, move, 25))
            ADJUST_SCORE(BEST_DAMAGE_MOVE);
        }
        if (gBattleMons[battlerAtk].status1 & STATUS1_TOXIC_POISON)
            ADJUST_SCORE(-1);
    ...
    */
    ...
    case EFFECT_LEECH_SEED:
        ...
        //  || HasMoveWithEffect(battlerDef, EFFECT_RAPID_SPIN)
          ...
          || aiData->abilities[battlerDef] == ABILITY_MAGIC_GUARD
          || aiData->abilities[battlerDef] == ABILITY_MAGIC_BOUNCE)
            break;
        else
            ADJUST_SCORE(BEST_DAMAGE_MOVE);
        break;
    ...
    case EFFECT_TELEPORT: // Either remove or add better logic
        ...
        if (CanTargetFaintAi(battlerDef, battlerAtk))
        ...
        else if (!gDisableStructs[battlerAtk].isFirstTurn)
            {
                ...
                if (AI_IsFaster(battlerAtk, battlerDef, move, predictedMoveSpeedCheck, DONT_CONSIDER_PRIORITY)
                && (AIHitsToKOPlayerMaxRoll <= PlayerHitsToKOAIMaxRoll)
                && (AIHitsToKOPlayerMaxRoll != 0)
                && (AIHitsToKOPlayerMaxRoll != 1))
                    ADJUST_SCORE(-1);
                else if (!AI_IsFaster(battlerAtk, battlerDef, move, predictedMoveSpeedCheck, DONT_CONSIDER_PRIORITY)
                && (AIHitsToKOPlayerMaxRoll < PlayerHitsToKOAIMaxRoll)
                && (AIHitsToKOPlayerMaxRoll != 0)
    ...
    //uq4_12_t effectiveness = aiData->effectiveness[battlerAtk][battlerDef][movesetIndex];
        ...
        else if (AIHitsToKOPlayerMaxRoll > PlayerHitsToKOAIMaxRoll)
        {
            if (RandomPercentage(RNG_AI_CUSTOM_AI_FIFTY_PERCENT, CUSTOM_AI_FIFTY_PERCENT))
                ADJUST_SCORE(+2);
        }
    ...
    */
    case EFFECT_STOCKPILE:
        ...
        if (shouldSetUp)
        {
            if (gBattleMons[battlerAtk].statStages[STAT_DEF] <= DEFAULT_STAT_STAGE
    ...
    /*
        ...
        if (aiData->abilities[battlerDef] == ABILITY_CONTRARY)
            ADJUST_SCORE(GOOD_EFFECT);
    */    
        if (aiData->items[battlerAtk] == ITEM_MIRROR_HERB)
            ADJUST_SCORE(+1);
        IncreaseConfusionScore(battlerAtk, battlerDef, move, &score);
        break;
    case EFFECT_FLATTER:
    ...
    */  u32 partnerHitsToKOPlayer1 = GetBestNoOfHitsToKO(battlerPartnerAtk, BATTLE_OPPOSITE(battlerAtk), AI_ATTACKING);
    ...
    case EFFECT_GRUDGE:
        if (CanTargetFaintAi(battlerDef, battlerAtk))
        {
            if (AI_IsFaster(battlerAtk, battlerDef, move, predictedMoveSpeedCheck, DONT_CONSIDER_PRIORITY))
                ADJUST_SCORE(BEST_DAMAGE_MOVE);
            else if (aiData->abilities[battlerAtk] == ABILITY_PRANKSTER)
                ADJUST_SCORE(BEST_DAMAGE_MOVE);    
        } 
        break;
    case EFFECT_SNATCH:
    ...
    case EFFECT_SHELL_SMASH:
        ...
        {
            if (GetBestDmgFromBattler(battlerAtk, battlerDef, AI_DEFENDING) * 1.5 <= gBattleMons[battlerAtk].hp)
                ADJUST_AND_RETURN_SCORE(NO_DAMAGE_OR_FAILS);
            else 
                ADJUST_SCORE(DECENT_EFFECT); 
        }   
    ...
    */
    ...
    case EFFECT_FLING:
        ...
        else if (aiData->holdEffects[battlerAtk] == HOLD_EFFECT_FLINCH)
        {
            ...
            else if (!(gAiLogicData->abilities[battlerDef] == ABILITY_SHIELD_DUST) 
            || (!(gAiLogicData->abilities[battlerDef] == ABILITY_INNER_FOCUS)))
                {
                    ADJUST_AND_RETURN_SCORE(NO_DAMAGE_OR_FAILS);
                }
    ...
    case EFFECT_POWDER:
        if (HasMoveWithType(battlerDef, TYPE_FIRE))
            {
                ADJUST_SCORE(BEST_DAMAGE_MOVE);
            if (aiData->hpPercents[battlerDef] <= 25
            && (RandomPercentage(RNG_AI_CUSTOM_AI_FIFTY_PERCENT, CUSTOM_AI_FIFTY_PERCENT)))
                ADJUST_SCORE(DECENT_EFFECT);
            }
        break;
    ...
    case EFFECT_HEAL_BLOCK:
        ...
        break;
    case EFFECT_SOAK:
    // Bazzo Note: remaking this to affect magic powder, as only that and NOT soak are used in sheesh silver
    /*
    ...
    */
    ...
    case EFFECT_QUASH:
        ...
        if (hasPartner
        && (AI_IsSlower(BATTLE_PARTNER(battlerAtk), battlerDef, aiData->partnerMove, predictedMoveSpeedCheck, DONT_CONSIDER_PRIORITY))
...
// bazzodamagespeeddropai
    ...
    */
    ...
    case EFFECT_SHORE_UP:
        ...
        if (gBattleMons[battlerAtk].status1 & STATUS1_TOXIC_POISON)
            ADJUST_SCORE(-1);
        break;
        //Bazzo note: not sure why this isn't just treated as a damaging move? probably need to check that it is later...
        /*
    ...
    case EFFECT_SMACK_DOWN:
        if (!AI_IsBattlerGrounded(battlerDef) && !IsBestDmgMove(battlerAtk, battlerDef, AI_ATTACKING, move))
            {
                if (HasDamagingMoveOfType(battlerAtk, TYPE_GROUND) || HasMoveWithEffect(battlerAtk, EFFECT_OHKO)) //has damaging ground move or fissure
                ADJUST_SCORE(BEST_DAMAGE_MOVE);
            }
        break;
    case EFFECT_KNOCK_OFF:
    ...
    */
        if (!IsBestDmgMove(battlerAtk, battlerDef, AI_ATTACKING, move)
        && (CanKnockOffItem(battlerDef, aiData->items[battlerDef]))
        && (GetNoOfHitsToKOBattler(battlerAtk, battlerDef, gAiThinkingStruct->movesetIndex, AI_ATTACKING) > 2)
        && (GetNoOfHitsToKOBattler(battlerAtk, battlerDef, gAiThinkingStruct->movesetIndex, AI_DEFENDING) > 2))
            ADJUST_SCORE(BEST_DAMAGE_MOVE);
        break;
        //Bazzo note: removing this, seems silly
    /*
...
static s32 AI_CalcAdditionalEffectScore(u32 battlerAtk, u32 battlerDef, u32 move, struct AiLogicData *aiData)
{
    ...
    for (i = 0; i < additionalEffectCount; i++)
    {
        ...
        if (additionalEffect->self)
        {
            ...
            else if (aiData->abilities[battlerAtk] == ABILITY_CONTRARY)
            {
                ...
                case MOVE_EFFECT_SP_ATK_MINUS_2:
                    if (!IsBestDmgMove(battlerAtk, battlerDef, AI_ATTACKING, move))
                        {
                            StageStatId = STAT_CHANGE_SPATK + additionalEffect->moveEffect - MOVE_EFFECT_SP_ATK_PLUS_2;
                            ADJUST_SCORE(IncreaseStatUpScore(battlerAtk, battlerDef, StageStatId, move));
                        }
                    break;
                ...
                break;
            /*
            { //Bazzo note: TODO need to think on this and whether contrary moves being treated as setup is a good idea or just invites chaotic scoring...
                switch (additionalEffect->moveEffect)
                {
        ...
        else // consider move effects that hinder the target bazzodamagespeeddropai
        {
            ...
            if (!IsBestDmgMove(battlerAtk, battlerDef, AI_ATTACKING, move))
            {
                ...
                //case MOVE_EFFECT_EVS_MINUS_2:
                    ...
                    //    ADJUST_SCORE(DECENT_EFFECT);
                    //break;
                //Bazzo Note: moved to end with other status secondary effects
                //case MOVE_EFFECT_POISON:
                //    IncreasePoisonScore(battlerAtk, battlerDef, move, &score);
                //    break;
                ...
                case MOVE_EFFECT_INCINERATE:
                    if (gBattleMons[battlerDef].volatiles.substitute || aiData->abilities[battlerDef] == ABILITY_STICKY_HOLD)
                        ...
                        ADJUST_SCORE(BEST_DAMAGE_MOVE);
                    break;
                case MOVE_EFFECT_STEALTH_ROCK: //Bazzo note: only gmax stonesurge, should be fine
                    if (AI_ShouldSetUpHazards(battlerAtk, battlerDef, move, aiData));
                    {
                        if (gDisableStructs[battlerAtk].isFirstTurn)
                            ADJUST_SCORE(BEST_EFFECT);
                ...
                case MOVE_EFFECT_WRAP:
                    ...
                    && (!IsBestDmgMove(battlerAtk, battlerDef, AI_ATTACKING, move)))
                    { 
                        if (GetBestDmgFromBattler(battlerDef, battlerAtk, AI_DEFENDING) * 2 < gBattleMons[battlerAtk].hp)
                            ADJUST_SCORE(WEAK_EFFECT);
                        else
                            ADJUST_SCORE(BEST_DAMAGE_MOVE);
                    }
