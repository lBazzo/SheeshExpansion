...
static bool32 Ai_AttackerMovesAfterTarget(struct DamageContext *ctx)
{
    return GetAiTurnOrder(ctx->aiTurnOrder, ctx->battlerAtk) > GetAiTurnOrder(ctx->aiTurnOrder, ctx->battlerDef);
}

static bool32 Ai_AttackerMovesLast(struct DamageContext *ctx)
{
...
bool32 TryFieldEffects(enum FieldEffectCases caseId)
{
    ...
    case FIELD_EFFECT_TRAINER_STATUSES:  // starting field/side/etc statuses with a variable
        ...
        else if (gStartingStatuses.magicRoom || gStartingStatuses.magicRoomTemporary)
        {
            ...
            gStartingStatuses.trickRoomTemporary = gStartingStatuses.magicRoom = FALSE;
        }
...
static inline u32 CalcMoveBasePower(struct DamageContext *ctx)
{
    ...
    case EFFECT_REVENGE:
        ...
        else if (gProtectStructs[battlerAtk].revengeDoubled & 1u << battlerDef)
            basePower *= 2;
        break;
...
static inline u32 CalcAttackStat(struct DamageContext *ctx)
{
    ...
    case ABILITY_ROCKY_PAYLOAD:
        if (moveType == TYPE_ROCK)
            modifier = uq4_12_multiply(modifier, UQ_4_12(1.5));
        break;
    case ABILITY_THREATENING_AURA:
        if (moveType == TYPE_DARK)
            modifier = uq4_12_multiply(modifier, UQ_4_12(1.5));
        break;
    case ABILITY_FIERY_DEMEANOUR:
        if (moveType == TYPE_FIRE)
            modifier = uq4_12_multiply(modifier, UQ_4_12(1.5));
        break;
    case ABILITY_DRAGON_HERITAGE:
        if (moveType == TYPE_DRAGON)
            modifier = uq4_12_multiply(modifier, UQ_4_12(1.5));
        break;
    case ABILITY_BUG_EYED:
        if (moveType == TYPE_BUG)
            modifier = uq4_12_multiply(modifier, UQ_4_12(1.5));
        break;
    case ABILITY_AMPHIBIAN:
        if (moveType == TYPE_WATER)
            modifier = uq4_12_multiply(modifier, UQ_4_12(1.5));
        break;
    case ABILITY_PROTOSYNTHESIS:
