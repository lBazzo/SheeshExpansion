...
const struct TrainerClass gTrainerClasses[TRAINER_CLASS_COUNT] =
{
    ...
    [TRAINER_CLASS_HEX_MANIAC] = { _("Medium") },
    ...
    [TRAINER_CLASS_LEADER_KANTO] = { _("Leader") },
...
s32 GetBattleMovePriority(u32 battler, enum Ability ability, u32 move)
{
    ...
    else if (ability == ABILITY_SKITTER_SPEED
          && (GetConfig(CONFIG_GALE_WINGS) < GEN_7 || IsBattlerAtMaxHp(battler))
          && (GetMoveType(move) == TYPE_BUG || GetMoveType(move) == TYPE_POISON))
    {
        priority++;
    }
