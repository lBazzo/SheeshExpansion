            ...
            for (j = 0; j < IN_BOX_COUNT; j++)
            {
                ...
                && !GetBoxMonDataAt(i, j, MON_DATA_MARKINGS))
                {
                    ...
                    if (itemId != ITEM_NONE && AddBagItem(itemId, 1))
                    {
                        ...
                        SetMonFormPSS(&gPokemonStoragePtr->boxes[i][j], FORM_CHANGE_ITEM_HOLD);
                    }
                }
            }
        }
        ...
        for (i = 0; i < PARTY_SIZE; i++)
        {
            ...
            && !GetMonData(&gPlayerParty[i], MON_DATA_MARKINGS))
            {
                ...
                if (itemId != ITEM_NONE && AddBagItem(itemId, 1))
                {
                    ...
                    SetMonFormPSS(&gPlayerParty[i].box, FORM_CHANGE_ITEM_HOLD);
                }
            }
        }
    }
...
{
    struct Pokemon *partyPokemon = gPlayerParty;

    u16 species = GetMonData(partyPokemon, MON_DATA_SPECIES);
    ...
    if (partyPosition == 0)
        ...
        sStorage->partySprites[partyPosition] = CreateMonIconSprite(species, personality, 152,  8 * (3 * (partyPosition - 1)) + 16, 1, 12);
