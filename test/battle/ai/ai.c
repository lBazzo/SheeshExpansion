...
{
    //KNOWN_FAILING; // Bazzo note: removed first turn only AI - apparently passes now for whatever reason
    GIVEN {
...
AI_SINGLE_BATTLE_TEST("AI has a chance to prioritize last chance priority damage over slow KO")
{
    ...
    GIVEN {
        ...
        OPPONENT(SPECIES_FLOATZEL) { Level(90); Speed(1); HP(1); Moves(MOVE_AQUA_JET, MOVE_WAVE_CRASH); }
    } WHEN {
        TURN { MOVE(player, MOVE_CELEBRATE); 
            SCORE_EQ_VAL(opponent, MOVE_AQUA_JET, 112); 
            SCORE_EQ_VAL(opponent, MOVE_WAVE_CRASH, 112);
        }
    }
}
...
AI_SINGLE_BATTLE_TEST("Bolt Beak damage will be correctly seen by AI (singles)")
{
    ...
    GIVEN {
        AI_FLAGS(AI_FLAG_CHECK_BAD_MOVE | AI_FLAG_CHECK_VIABILITY | AI_FLAG_TRY_TO_FAINT | AI_FLAG_OMNISCIENT | AI_FLAG_PREFER_HIGHEST_DAMAGE_MOVE);
        PLAYER(SPECIES_STARMIE) { Speed(playerSpeed); Moves(MOVE_PROTECT, MOVE_CELEBRATE); Level(100); }
        OPPONENT(SPECIES_ZAPDOS) { Speed(aiSpeed); Moves(MOVE_BOLT_BEAK, MOVE_THUNDER); Level(99); }
    } WHEN {
