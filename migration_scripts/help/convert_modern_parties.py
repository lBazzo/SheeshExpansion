...
begin_party_definition   = re.compile(r'static const struct TrainerMon[[a-zA-Z]+ (\w+)\[\] = .*')
...
ivs_definition           = re.compile(r'\.iv = ([0-9]+)')
    ...
    for line_no, line in enumerate(in_h, 1):
        try:
            ...
            if m := begin_party_definition.search(line):
                ...
                [hp, attack, defense, speed, special_attack, special_defense] = [ math.floor(int(m.groups()[0]) / 8)] * 6
...
trainer_ai_flag_definition         = re.compile(r'AI_SCRIPT_(\w+)')
trainer_party_definition           = re.compile(r'\.party = [a-zA-Z-_]+[a-zA-Z-_0-9]*\((\w+)\)')
